...
interface AppConfig {
  ...
  viz: { host: string; port: number; scene_path: string; scroll_speed: number; auto_pause_on_idle: boolean; auto_pause_on_idle_seconds: number };
}
```
